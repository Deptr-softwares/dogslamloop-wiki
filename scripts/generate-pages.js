...
const { sharedAssetVersion, stampSharedAssets } = require('./asset-version.js');
...
function buildPages(nav, previews = {}, archived = {}, toolScripts = new Set(), toolStyles = new Set()) {
    ...
    for (const [category, entries] of Object.entries(nav)) {
        ...
        for (const entry of entries) {
            ...
            // Stamped here, not in the templates, so all four stub builders
            // and the tombstone below get it from one place - and so this
            // script and scripts/stamp-assets.js can never disagree about the
            // current version and fail `npm run validate` against each other.
            html = stampSharedAssets(html, sharedAssetVersion());
