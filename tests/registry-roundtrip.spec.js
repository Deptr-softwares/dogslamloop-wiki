...
const { buildNavigation, validateNavigation, NAV_FLAGS } = require('../scripts/fetch-registry.js');
...
//
// The flags are reversed out of NAV_FLAGS rather than listed again. Listing
// them again is what broke this: is_hidden shipped in v0.16, buildNavigation
// emitted isHidden, and this fixture never rebuilt it - so the round-trip was
// guaranteed to fail the first time a page was genuinely hidden. It stayed
// green for eleven days because no page was, then failed on 2026-09-05 and
// took the regeneration job down with it, since that job runs the suite
// before it commits. Deriving the mapping means the next flag cannot repeat it.
function registryRowsFromNav(navigation) {
  ...
  for (const [category, items] of Object.entries(navigation)) {
    ...
    for (const e of items) {
      ...
      const flags = {};
      for (const [column, key] of Object.entries(NAV_FLAGS)) flags[column] = !!e[key];
      rows.push({
...
test('every boolean flag in the committed navigation comes from NAV_FLAGS', () => {
  // The round-trip above now derives its fixture from NAV_FLAGS, so the two
  // sides cannot disagree - unless someone writes a flag into buildNavigation
  // by hand instead of adding it to the map, which is exactly the shape of the
  // original bug. This reads the committed file and asserts every `true` in it
  // is a key the map knows, so that bypass fails here with a name rather than
  // as an unreadable byte-for-byte diff.
  const known = new Set(Object.values(NAV_FLAGS));
  const structural = new Set(['id', 'name', 'url', 'cms_config', 'archetype', 'tier', 'releaseDate']);
  ...
  for (const entries of Object.values(nav)) {
    for (const entry of entries) {
      for (const [key, value] of Object.entries(entry)) {
        ...
        if (value === true && !known.has(key)) unknown.add(key);
      }
    }
  }
  expect([...unknown], 'add it to NAV_FLAGS in scripts/fetch-registry.js').toEqual([]);
});
